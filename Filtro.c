...
uint8* applyFilter(uint8* img, float* filtro, uint32 imgW, uint32 imgH, uint32 filW, uint32 filH, uint32 numThreads)
{
	...
	uint8* amplifiedImg = (uint8*)malloc(3 * (imgW + 2)*(imgH + 2));
...
unsigned __stdcall threadApplyFilter(void* params)
{
	...
	uint8 *xmm_pixOrig = (uint8*)_aligned_malloc(sizeof(uint8) * 16, 16);
	uint8 *xmm_pixResult = (uint8*)_aligned_malloc(sizeof(uint8) * 16, 16);
	...
	_aligned_free(xmm_pixOrig);
	_aligned_free(xmm_pixResult);
